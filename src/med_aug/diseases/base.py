    ...
    def get_taking_variables(self) -> List[str]:
        ...
        for drug_class in self.drug_classes:
            ...
            for keyword in drug_class.keywords[:1]:  # Use first keyword as primary
                variable = f"taking_{keyword.lower().replace(' ', '_').replace('-', '_')}"
...
class DiseaseModuleConfig(BaseModel):
    ...
    drug_classes: List[Dict[str, Any]] = Field(..., description="Drug class configurations")
    web_sources: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Web source configurations")

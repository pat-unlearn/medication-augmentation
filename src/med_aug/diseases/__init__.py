...
class DiseaseRegistry:
    ...
    def _discover_modules(self) -> None:
        ...
        for finder, module_name, ispkg in pkgutil.iter_modules(package_path):
            if (
                module_name != "base" and ispkg
            ):  # Skip base module and only look at packages
                try:
                    ...
                    if hasattr(module, "MODULE_CLASS"):
                        ...
                        if isinstance(disease_class, type) and issubclass(
                            disease_class, DiseaseModule
                        ):
                            ...
                            logger.warning(
                                f"MODULE_CLASS in {module_name} is not a DiseaseModule subclass"
                            )
                    else:

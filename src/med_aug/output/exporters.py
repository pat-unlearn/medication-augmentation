        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            for sheet_name, sheet_data in data.items():
                if isinstance(sheet_data, list) and sheet_data:
                    ...
                    logger.warning("excel_export_unsupported_data_type",
                                 sheet=sheet_name,
        ...
        for section_name, section_data in data.items():
            ...
            if isinstance(section_data, list) and section_data and isinstance(section_data[0], dict):
                ...
                if len(section_data) > 100:
                    html += f'<p><em>Showing first 100 of {len(section_data)} rows</em></p>'
                    ...
                    if isinstance(section_data, list) and section_data:
                        ...
                        if isinstance(section_data[0], dict):
                            ...
                            for row in section_data[:50]:  # Limit rows
                                table_data.append([str(row.get(k, '')) for k in section_data[0].keys()])
                        else:

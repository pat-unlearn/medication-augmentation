...
class TestDrugClassConfig:
    ...
    def test_invalid_confidence_threshold(self):
        ...
        # Too high
        with pytest.raises(
            ValueError, match="Confidence threshold must be between 0 and 1"
        ):
            DrugClassConfig(
                name="test",
                keywords=["drug"],
    ...
    def test_empty_keywords_validation(self):
        ...
        with pytest.raises(ValueError, match="must have at least one keyword"):
            DrugClassConfig(
                name="test", keywords=[], confidence_threshold=0.8, web_sources=["fda"]
            )
...
class TestDiseaseModule:
    ...
    def test_get_confidence_threshold(self, concrete_disease_module):
        ...
        default_threshold = concrete_disease_module.get_confidence_threshold(
            "nonexistent"
        )
    ...
    def test_validate_medication(self, concrete_disease_module):
        ...
        assert (
            concrete_disease_module.validate_medication("Drug2", "class1") is True
        )  # Case insensitive
...
class TestDiseaseModuleConfig:
    ...
    def test_disease_module_config_creation(self):
        ...
        config = DiseaseModuleConfig(
            ...
            web_sources={"fda": {"base_url": "http://fda.gov", "rate_limit": 1.0}},
            llm_settings={"model": "gpt-4", "temperature": 0.1},
            validation={"require_human_review": True},
        )

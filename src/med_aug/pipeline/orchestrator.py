...
from .phases import (
    PipelinePhase, PhaseStatus, PhaseResult,
    DataIngestionPhase, ColumnAnalysisPhase, MedicationExtractionPhase,
    WebResearchPhase, ValidationPhase, LLMClassificationPhase, OutputGenerationPhase
)
...
class PipelineOrchestrator:
    ...
    def __init__(self, config: PipelineConfig, pipeline_id: Optional[str] = None):
        ...
        # Initialize managers
        self.checkpoint_manager = CheckpointManager() if config.enable_checkpoints else None
    ...
    def _initialize_phases(self) -> List[PipelinePhase]:
        ...
        logger.debug("phases_initialized", count=len(phases), names=[p.name for p in phases])
        return phases
            ...
            for phase in self.phases:
                ...
                if result.status == PhaseStatus.COMPLETED:
                    ...
                    if self.config.enable_checkpoints:
                        if len(self.completed_phases) % self.config.checkpoint_interval == 0:
                ...
                for name, result_dict in checkpoint.phase_results.items():
                    if name in self.completed_phases:
                        ...
                        self.phase_results[name] = PhaseResult(
                            ...
                            end_time=datetime.fromisoformat(result_dict['end_time']) if result_dict.get('end_time') else None,

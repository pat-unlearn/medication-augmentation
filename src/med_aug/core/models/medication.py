...
class MedicationClassification(BaseModel):
    ...
    alternative_classes: List[str] = Field(default_factory=list, description="Alternative possible classes")
...
class WebResearchResult(BaseModel):
    ...
    generic_names: List[str] = Field(default_factory=list, description="Generic names found")
    brand_names: List[str] = Field(default_factory=list, description="Brand names found")
    drug_class_hints: List[str] = Field(default_factory=list, description="Drug class hints from sources")
    mechanism_of_action: Optional[str] = Field(None, description="Mechanism of action")
    fda_approval_date: Optional[str] = Field(None, description="FDA approval date if available")
    clinical_trials: List[str] = Field(default_factory=list, description="Related clinical trials")
    ...
    def has_sufficient_data(self) -> bool:
        ...
        return (
            len(self.generic_names) > 0 or 
            len(self.brand_names) > 0 or 
            len(self.drug_class_hints) > 0
        )
...
class PipelineState(BaseModel):
    ...
    current_phase: str = Field(default="initialization", description="Current pipeline phase")
    phases_completed: List[str] = Field(default_factory=list, description="Completed phases")
    medications_processed: int = Field(default=0, description="Number of medications processed")
    medications_classified: int = Field(default=0, description="Number of medications classified")

...
class TestDataAnalyzer:
    ...
    def sample_medication_df(self):
        ...
        return pd.DataFrame({
            ...
            'AGENT': ['pembrolizumab', 'Keytruda', 'osimertinib', 'Tagrisso', 'carboplatin'],
    ...
    def mixed_column_df(self):
        ...
        return pd.DataFrame({
            'med_name': ['aspirin', 'ibuprofen', 'acetaminophen', 'metformin', 'lisinopril'],
            'drug_text': ['Take aspirin daily', 'Ibuprofen 400mg', 'Tylenol as needed', 'Metformin 1000mg', 'Lisinopril 10mg'],
            'treatment': ['pain relief', 'inflammation', 'fever', 'diabetes', 'hypertension'],
            'patient_name': ['John Doe', 'Jane Smith', 'Bob Johnson', 'Alice Brown', 'Charlie Davis'],
            'notes': ['stable', 'improving', 'monitoring', 'stable', 'follow-up needed']
        })
    ...
    def test_column_name_scoring(self, analyzer):
        ...
        assert analyzer._score_column_name('chemo_drug') == 1.0  # Contains 'drug' so scores 1.0
    ...
    def test_pattern_scoring(self, analyzer):
        ...
        med_values = ['pembrolizumab', 'osimertinib', 'carboplatin', 'paclitaxel', 'nivolumab']
    ...
    def test_content_scoring(self, analyzer):
        ...
        cap_values = ['Pembrolizumab', 'Nivolumab', 'Osimertinib', 'Carboplatin', 'Paclitaxel']
    ...
    def test_sample_medication_extraction(self, analyzer):
        ...
        values = [
            'pembrolizumab', 'patient1', 'osimertinib', 'test',
            'carboplatin', 'data', 'nivolumab', 'value',
            'atezolizumab', 'info', 'durvalumab', 'sample'
        ]
    ...
    def test_analyze_null_column(self, analyzer):
        ...
        null_df = pd.DataFrame({
            'all_null': [None, np.nan, None, np.nan, None]
        })
    ...
    def test_confidence_threshold(self, analyzer, sample_medication_df):
        ...
        low_threshold_results = analyzer.analyze_dataframe(sample_medication_df, confidence_threshold=0.1)
        
        # High threshold - should get fewer results
        high_threshold_results = analyzer.analyze_dataframe(sample_medication_df, confidence_threshold=0.8)

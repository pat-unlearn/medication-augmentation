...
class TestMedicationExtractor:
    ...
    def test_normalization(self, extractor):
        ...
        assert (
            extractor._normalize_medication("osimertinib 80mg tablets") == "Osimertinib"
        )
    ...
    def test_deduplication(self, extractor):
        ...
        assert (
            len(deduplicated) <= 3
        )  # pembrolizumab, nivolumab, and possibly one variant
    ...
    def test_medication_type_determination(self, extractor):
        ...
        # Brand names
        assert (
            extractor._determine_medication_type("keytruda", ["Keytruda"])
            == MedicationType.BRAND
        )
        assert (
            extractor._determine_medication_type("opdivo", ["Opdivo"])
            == MedicationType.BRAND
        )

        # Combinations
        assert (
            extractor._determine_medication_type("Carboplatin/Paclitaxel", [])
            == MedicationType.COMBINATION
        )
        assert (
            extractor._determine_medication_type("Drug1+Drug2", [])
            == MedicationType.COMBINATION
        )

        # Clinical trial
        assert (
            extractor._determine_medication_type("ABC-123", [])
            == MedicationType.CLINICAL_TRIAL
        )
        assert (
            extractor._determine_medication_type("XYZ-4567", [])
            == MedicationType.CLINICAL_TRIAL
        )

        # Abbreviations
        assert (
            extractor._determine_medication_type("EGFR", [])
            == MedicationType.ABBREVIATION
        )
        assert (
            extractor._determine_medication_type("PD-L1", [])
            == MedicationType.ABBREVIATION
        )

        # Generic (default)
        assert (
            extractor._determine_medication_type("pembrolizumab", [])
            == MedicationType.GENERIC
        )
        assert (
            extractor._determine_medication_type("osimertinib", [])
            == MedicationType.GENERIC
        )
    ...
    def test_parenthetical_content_removal(self, extractor):
        ...
        assert (
            extractor._normalize_medication("pembrolizumab (Keytruda)")
            == "Pembrolizumab"
        )
        ...
        assert (
            extractor._normalize_medication("medication [extra data]") == "Medication"
        )

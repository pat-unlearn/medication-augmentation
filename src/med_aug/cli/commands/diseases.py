...
def show_keywords(
    ...
    drug_class: Optional[str] = typer.Option(None, "--class", "-c", help="Filter by drug class")
) -> None:
    ...
    if drug_class:
        ...
        if not config:
            console.print(f"❌ Drug class '{drug_class}' not found in {disease}", style="red")
            console.print(f"Available classes: {', '.join([dc.name for dc in module.drug_classes])}")
            raise typer.Exit(1)
    ...
    if not module.drug_classes:
        ...
        for dc in module.drug_classes:
            if not dc.keywords:
                ...
                warnings.append(f"Drug class '{dc.name}' has only {len(dc.keywords)} keywords")
            
            if not (0 <= dc.confidence_threshold <= 1):
                issues.append(f"Drug class '{dc.name}' has invalid confidence threshold: {dc.confidence_threshold}")

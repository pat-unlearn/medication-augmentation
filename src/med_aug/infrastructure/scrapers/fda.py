...
class FDAScraper(BaseScraper):
    ...
    async def _search_openfda_drugs(self, medication_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for search_term in search_terms:
                ...
                async with self.rate_limit():
                    response = await self.client.get(url, headers={"User-Agent": self.config.user_agent})
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning("fda_api_error", status=response.status_code, url=url)
                        
            return None
            
        except Exception as e:
            logger.error("openfda_search_failed", medication=medication_name, error=str(e))
            return None
    
    async def _search_orange_book(self, medication_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.rate_limit():
                ...
                if response.status_code == 200:
                    ...
                    logger.warning("orange_book_api_error", status=response.status_code, url=url)
                    
            return None
            
        except Exception as e:
            logger.error("orange_book_search_failed", medication=medication_name, error=str(e))
            return None
    
    def _parse_openfda_result(self, result: Dict[str, Any], medication_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "source": "OpenFDA",
            "fda_application_number": openfda.get("application_number", [""])[0] if openfda.get("application_number") else "",
            "active_ingredient": openfda.get("generic_name", [""])[0] if openfda.get("generic_name") else "",
    ...
    def _parse_orange_book_result(self, result: Dict[str, Any], medication_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_ingredient": products.get("active_ingredients", [{}])[0].get("name", "") if products.get("active_ingredients") else "",
            "brand_names": [products.get("brand_name", "")] if products.get("brand_name") else [],
            "generic_names": [products.get("generic_name", "")] if products.get("generic_name") else [],
            "manufacturer": result.get("sponsor_name", ""),
            "dosage_forms": [products.get("dosage_form", "")] if products.get("dosage_form") else [],
            "strength": [products.get("active_ingredients", [{}])[0].get("strength", "")] if products.get("active_ingredients") else [],
